...
class Ui_AGEseq(object):
    def setupUi(self, AGEseq):
        ...
        self.goalfile_button.setGeometry(QtCore.QRect(50, 120, 121, 41))
        font = QtGui.QFont()
        font.setFamily("Agency FB")
        font.setPointSize(14)
        ...
        self.run_button.setGeometry(QtCore.QRect(50, 210, 121, 41))
        font = QtGui.QFont()
        font.setFamily("Agency FB")
        font.setPointSize(14)
        ...
        self.file_input.setGeometry(QtCore.QRect(180, 50, 251, 41))
        self.file_input.setObjectName("file_input")
        self.goalfile_input = QtWidgets.QTextBrowser(self.centralwidget)
        self.goalfile_input.setGeometry(QtCore.QRect(180, 120, 256, 41))
        self.goalfile_input.setObjectName("goalfile_input")
        self.run_out = QtWidgets.QTextBrowser(self.centralwidget)
        self.run_out.setGeometry(QtCore.QRect(180, 190, 256, 81))
        self.run_out.setObjectName("textBrowser_3")
    ...
    def retranslateUi(self, AGEseq):
        ...
        self.file_button.setText(_translate("AGEseq", "选择文件"))
...
class MainUi(QtWidgets.QMainWindow, Ui_AGEseq):
    ...
    def selfile(self):
        self.selfile_input = self.fileDialog.getOpenFileName()
    ...
    def run(self):
        ...
        self.file_out = self.fileDialog.getSaveFileName()
        out += "".join(self.file_out)+"\n"
        self.run_out.setText(out)
        script, dat_dir, file_design, final_out = argv, "reads", "targets.txt", "AGE_output"
        ...
        ########################################################
        ##  step3 to step 5 into one loop
        try:
            REPORT = open(final_out, 'w')
        except IOError as e:
            sys.exit("Cannot write report file\n")
        ...
        for file_blat_in in fasta_files:
            ...
            read_num_address = PR.MAIN(file_blat_in, bed_hash_address, final_out,get_buf,write_buf,total_read_num,
                                       design_hash,get_ed_note,get_ali,tri_seq,mismatch_cutoff,tri_in,min_cutoff,
                                       indel_report)  ## address of one hash
...
class PslResult(AutoVivification):
    ...
    def get_reverse(parameter_in):
        ...
        for indel_each in indel_in:
            ...
            if re_match:
                ooo = str(len_temp - re_match.group(1) - re_match.group(3)) + re_match.group(2) + re_match.group(3)
                if re_match.group(2) == 'I':
                    ooo = str(len_temp - re_match.group(1)) + re_match.group(2) + re_match.group(3)
...
##get_reverse
    ...
    def get_alignment(blocks, blockLengths, qStarts, tStarts, query_seq, target_seq,treat_sequence,treat_inter):
        ...
        ### after blocks
        try:
            after_q = query_seq[(qArray[-1] + blockSizes[-1]):]
            after_t = target_seq[(tArray[-1] + blockSizes[-1]):]
        ...
        if int(blocks) > 1:
            for i in range(int(blocks) - 1):
                ...
                if len(inter_t_seq) - len(inter_q_seq) > 0:
                    ...
                    out = str(qArray[i] + blockSizes[i] + len(inter_q_seq)) + 'I' + str(len(inter_t_seq) - len(inter_q_seq))
                if len(inter_q_seq) - len(inter_t_seq) > 0:
                    out = str(qArray[i] + blockSizes[i] + len(inter_t_seq)) + 'D' + str(len(inter_q_seq) - len(inter_t_seq))
        ...
        # print(out_alignment[0])
        return out_alignment


##   get_alignment

    def get_editing_note(address_in, query_seq, target_seq, full_read,get_alignment,tr_seq,tri_in):
        # target: part of read
        # query : original sequence in design file
...
## get_editing_note
    ...
    def get_out_buffer(fas_file_in, query_with_seq2num_in, seq2alignment_in,total_read_num,min_cutoff,design_hash,
        ...
        for ref_name in sorted(query_with_seq2num_in.keys()):
            ...
            other_record = []
            ## loop for records
            in_hash_sorted = sorted(in_hash.keys(), reverse=True)
            for part_of_read in in_hash_sorted:
                hitnum = in_hash[part_of_read]
                if int(hitnum) < min_cutoff:
                    continue
                if int(hitnum) / test_case_num < 0.00005:
                    continue
                ...
                if part_of_read in seq2alignment_in.keys():
                    ...
                    snp_note = ''
                    if snp_num > 0:
                        snp_note = str(snp_num) + ' SNP(' + ",".join("%s" % id for id in snp_out) + ")"
                    sub_hit_num = str(sub_hit_num) + hitnum
                    total_non_redun += int(hitnum)
                    ##output here
                    ...
                    if not re.search(r'no.+indel', edit_note):
                        if ((test_case_num > 500 and int(hitnum) <= 3) or (
                                int(hitnum) / test_case_num < 0.001)) and re.search(r'strange', edit_note):
                            continue  # filter strange case in deep sequencing with few reads
                        report_indel_count += 1
                        # total indel
                        indel_hit_num = indel_hit_num + int(hitnum)
...
##
        ...
        for fas_file_in in sorted(hash_out.keys()):
            ...
            for ref_name in sorted(hash_out[fas_file_in].keys()):
                if not hash_out[fas_file_in][ref_name]["data"]:
                    ...
                    continue
                date = hash_out[fas_file_in][ref_name]["data"]
                for da in date:
            ...
            for ref_name in sorted(hash_out[fas_file_in].keys()):
                if not hash_out[fas_file_in][ref_name]["data"]:
                    continue
                date = hash_out[fas_file_in][ref_name]["date"]
    ...
    def single_fasta2array(input_file, file_short):
        IN = open(input_file)
        id = file_short
        single_fas = []
        seq = ""
        ...
        id = id.replace("_", "")
        id = id + "_1"
        single_fas.append((id, seq))
        IN.close()


    def check_read_num(file_in):
        FAS_in = open(file_in)
        seq_num = 0
        for line in FAS_in:
            if line.startwith(">"):
    ...
    def MAIN(fas_file_in, bed_file_address_in, file,get_out_buffer,write_buffer_out,total_read_num,design_hash,
             get_editing_note,get_ali,tri_seq,mismatch_cutoff,tri_in,min_cutoff,indel_report):
        #########################################################
            ...
            if lineIn.startswith(">"):
                ...
                if id in bed_hash_in.keys():
                    ...
                    for read_key in bed_hash_in[id].keys():
                        ...
                        align_1 = list(align_ref)
                        align_2 = list(align_read)
                        site_snp_list = []
                        iden_num = 0
                    ...
                    if len(snp_filtering) / len("".join(ref_ori_seq)) > mismatch_cutoff:
                        pass
                    ...
                    if seq_out in seq_part2assignment.keys():
                        ...
                        query_with_seq2num[first_assignment][seq_out] += total_num
                        continue
        ...
        (hash_addr1, hash_addr2) = get_out_buffer(fas_file_in, query_with_seq2num, seq2alignment,total_read_num,
                                                  min_cutoff,design_hash,indel_report)
        ### write output
